...
- `--terminate` closes the connections the role can close. Closing one that belongs to another role needs membership in `pg_signal_backend`, or a superuser, which owning the database does not confer: a backend the role may not signal is left attached, and Postgres then settles the copy as it would have without the flag.
...
- `--terminate` closes open connections to the live database before copying it, one backend at a time, so a backend Postgres will not let the role signal costs that backend and no other. Postgres refuses to use a database as a template while anyone is connected, and dev servers keep pools open. Nothing is terminated unless you pass the flag. Without it, `apply` falls back to the snapshot, and `template create|refresh` stop with an error that says what is in the way. With it, the same two happen over a backend the role may not signal, because it is still attached when the copy runs: the message then says the flag was refused and names `pg_signal_backend` rather than offering it again, since stopping whatever is connected is the remedy that needs no privilege at all.
